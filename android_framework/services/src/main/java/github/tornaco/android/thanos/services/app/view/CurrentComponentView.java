...
public class CurrentComponentView extends LinearLayout {
                    ...
                    case MotionEvent.ACTION_MOVE:
                        if (!mLocked && inDragMode) {
                            ...
                            if ((dx * dx + dy * dy) > mTouchSlop) {
                                ...
                                mWm.updateViewLayout(CurrentComponentView.this, mLp);
                                return true;
                            }
                        }
                        break;
    ...
    public void detach() {
        try {
            mWm.removeViewImmediate(CurrentComponentView.this);
        } catch (Exception ignored) {
            Timber.wtf("Fail detach float view: " + Log.getStackTraceString(ignored));
        }
    }
