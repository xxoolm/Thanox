...
public class AliPayUtil {
    ...
    public static boolean startPay(Context context) {
        if (!PkgUtils.isPkgInstalled(context, ALIPAY_PACKAGE_NAME)) return false;
    ...
    public static boolean getRedPacket(Context context) {
        try {
            ...
            if (cmb != null) {
                cmb.setPrimaryClip(ClipData.newPlainText("RedPacket-Thanox", ALIPAY_RED_PACKET_CODE));
                Toast.makeText(context, R.string.module_donate_toast_alipay_red_packet_copied, Toast.LENGTH_SHORT).show();
